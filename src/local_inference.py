...
def get_concept_data(dataset: Type[Dataset], concept, model_id):
    tokenizer = AutoTokenizer.from_pretrained(model_id)
    tokenizer.pad_token = tokenizer.unk_token
    concept_dataset = dataset.filter(lambda x: x['concepts'] == concept)
    prompt_dataset = concept_dataset.map(lambda e: tokenizer(e['text'], truncation=False, padding='max_length'), batched=True)
    # return concept_dataset['text']
    return concept_dataset[:]['text']

def get_icl_text(text):
    ...
    for i in range(len(paragraphs)-2):
        j = i+2
        if j%2 == 1:
            answers = paragraphs[j][paragraphs[j].index('\n')+1:]
...
if __name__ == "__main__":
    ...
    # print(data)
    # print(get_concept_data(data, 'hg06', 'gpt2-large'))
    # print(get_concept_data(data, 'hg08', 'gpt2-large'))
    print(get_icl_text(get_concept_data(data, 'hg08', 'gpt2-large')[0]))
